...
export default function LeaderList({ title, leaders }: Props) {
  return (
    <div className="flex h-[283.45px] w-full flex-col rounded-[10px] border border-[#E0E8ED] bg-white p-4">
      <h3 className="text-[16px] font-bold leading-4 text-[#1D3557]">
        {title}
      </h3>
      <div className="mt-3 grid auto-rows-fr flex-1 divide-y divide-[#E0E8ED]">
        {leaders.map((leader) => (
          <div
            key={leader.name}
